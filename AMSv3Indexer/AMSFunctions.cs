...
namespace AMSv3Indexer
{
    public class AMSFunctions
    {
        ...
        [FunctionName(nameof(GetInsights))]
        public async Task<HttpResponseMessage> GetInsights([HttpTrigger(AuthorizationLevel.Function, "post", Route = null)] HttpRequest req, ILogger log)
        {
            _log = log;
            ...
            //1. Get the file from another storage account
            ...
            log.LogDebug($"Video to index {url}");

            var httpClient = new HttpClient();

            var uri = new Uri(url);
            var fileData = await httpClient.GetStreamAsync(uri);
            ...
            var jobName = $"job-{fileName}-{DateTime.Now.Ticks}";
            ...
            {
                insights = IsAdultCheck(outputAsset.Name);
            }

            return new HttpResponseMessage(System.Net.HttpStatusCode.OK) { Content = new StringContent(insights) };
        }
        ...
        private async Task DownloadOutputAssetAsync(string outputAssetName)
        {
            ...
            directory = Path.Combine(tempPath, outputAssetName);
        ...
        /// <param name="outputAssetName"></param>
        /// <returns></returns>
        private string IsAdultCheck(string outputAssetName)
        {
            var isAdult = false;
            //read the insighs.json
            // string insightsJson = File.ReadAllText($@"{OutputFolderName}\{outputAssetName}\insights.json");
            // string insightsJson = File.ReadAllText($@"{OutputFolderName}/{outputAssetName}/insights.json");
            string insightsJson = File.ReadAllText($@"{directory}/insights.json");
